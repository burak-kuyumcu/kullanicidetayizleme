...
<template>
  <div class="flex min-h-screen bg-[#F5F6FA] text-[#26303E] font-[Poppins,sans-serif]">
    ...
    <div class="w-[260px] bg-[#F5F6FA] border-r border-[#D8D9DD] flex flex-col">
      ...
      <nav v-if="selectedUserId" class="mt-15 flex-1 space-y-3">
        <RouterLink
          ...
          :class="route.name === 'user-todos'
            ? 'bg-white text-[#4F359B]'
            : 'text-[#6B6B6B]'"
        >
          <span
            class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 rounded-r-full"
            :class="route.name === 'user-todos' ? 'bg-[#4F359B]' : 'bg-transparent'"
          />
          <span
        ...
        <RouterLink
          :to="`/users/${selectedUserId}/posts`"
          class="relative flex items-center gap-3 py-3 pl-6 pr-4 text-base rounded-r-full transition"
          :class="route.name === 'user-posts'
            ? 'bg-white text-[#4F359B]'
            : 'text-[#6B6B6B]'"
        >
          <span
            class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 rounded-r-full"
            :class="route.name === 'user-posts' ? 'bg-[#4F359B]' : 'bg-transparent'"
          />
          <span
            class="w-8 h-8 flex items-center justify-center"
            :class="route.name === 'user-posts'
        ...
        <RouterLink
          :to="`/users/${selectedUserId}/gallery`"
          class="relative flex items-center gap-3 py-3 pl-6 pr-4 text-base rounded-r-full transition"
          :class="['user-gallery', 'user-album'].includes(route.name)
            ? 'bg-white text-[#4F359B]'
            : 'text-[#6B6B6B]'"
        >
          <span
      ...
      <nav v-else class="mt-8 flex-1">
        <RouterLink
          to="/"
          class="flex items-center gap-3 py-3 pl-6 pr-4 text-base rounded-r-full bg-white text-[#4F359B]"
        >
      ...
      <div class="mt-auto border-t border-[#D8D9DD] bg-[#F3F4F6]">
        <div class="px-6 py-5 flex items-center justify-center gap-3">
          <img
            ...
            alt="company"
            class="w-12 h-12 rounded-2xl object-cover bg-white shadow-sm"
          />
